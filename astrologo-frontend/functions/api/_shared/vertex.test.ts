...
describe('montagem das requisições REST do Vertex', () => {
  ...
  it('mapeia responseMimeType e responseJsonSchema para dentro do generationConfig', async () => {
    ...
    const schema = {
      ...
      properties: { html: { type: 'string' }, warnings: { type: 'array', items: { type: 'string' } } },
    };
    await client(sa, mock).models.generateContent({
      model: 'm',
      contents: 'x',
