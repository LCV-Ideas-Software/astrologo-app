...
- O inventário `THIRDPARTY` passa a refletir integralmente as 41 relações diretas dos manifestos raiz e frontend. A arquitetura official-first combina o License Compliance e o Dependency Review nativos do GitHub e o `build.license` do Vite com um contrato local fail-closed restrito à proveniência Swiss/WASM e à igualdade das cópias distribuídas; o bundle do navegador publica ainda `legal/BUNDLED-LICENSES.md` com os textos das licenças efetivamente incorporadas, sem alegar cobertura do bundle separado das Cloudflare Functions (#346).
