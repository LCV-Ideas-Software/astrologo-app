...
- O inventário `THIRDPARTY` passa a refletir integralmente as 33 dependências diretas do manifesto e as resoluções exatas do lockfile do frontend nas duas cópias distribuídas; um contrato local fail-closed no gate de Dependency Review impede versões ausentes, antigas, duplicadas ou extras e divergência entre as cópias (#346).
